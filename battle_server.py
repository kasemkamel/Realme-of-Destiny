...
AI_SEARCH_RADIUS = 75.0
FORMATION_SPACING = 15.0
ARRIVAL_THRESHOLD = 5.0
REFORM_IDLE_TIME = 3.0  # seconds a unit's soldiers should be idle before reforming
DEFAULT_DODGE_CHANCE = 0.10  # 10% dodge chance
...
class CombatSystem:
    ...
    def update(self, delta_time: float):
        if self.world.soldier_count == 0:
            return
        
        # تقليل وقت إعادة الهجوم
        self.world.attack_cooldowns = np.maximum(0, self.world.attack_cooldowns - delta_time)
        
        # استخدام Quadtree المركزي (بُني في بداية World.update)
        qt = self.world.quadtree
        if qt is None:
            return
        
        for i in range(self.world.soldier_count):
            if self.world.attack_cooldowns[i] > 0:
                continue
            
            if self.world.states[i] != UnitState.ATTACKING:
                continue
            
            pos = self.world.positions[i]
            attack_range = self.world.attack_ranges[i]
            
            # البحث عن أعداء في مدى الهجوم
            nearby = qt.query_radius((pos[0], pos[1]), attack_range)
            ...
            for x, y, idx in nearby:
                ...
                # البحث عن أعداء فقط من الفرق المعادية
                enemy_teams = []
                if self.world.teams[i] == Team.PLAYER:
                    enemy_teams = [Team.ENEMY]
                elif self.world.teams[i] == Team.ENEMY:
                    enemy_teams = [Team.PLAYER, Team.ALLY]
                elif self.world.teams[i] == Team.ALLY:
                    enemy_teams = [Team.ENEMY]
                
                if (self.world.teams[idx] in enemy_teams and 
                    self.world.healths[idx] > 0):
            ...
            if closest_enemy is not None:
                dodge = float(self.world.dodge_chances[closest_enemy])

                # نستخدم numpy RNG (أسرع وموحد مع البنى العددية)
                if np.random.rand() < dodge:
                    # صدّ الضربة — لا تُنقص الصحة، ولكن المهاجم يدخل في الـ cooldown
                    self.world.evade_counts[closest_enemy] += 1
                    self.world.attack_cooldowns[i] = DEFAULT_ATTACK_COOLDOWN
                    logger.debug(f"Soldier {closest_enemy} evaded attack from {i}")
                else:
                    # الإصابة تُطبق كما سابقاً
                    self.world.healths[closest_enemy] -= self.world.attack_damages[i]
                    self.world.attack_cooldowns[i] = DEFAULT_ATTACK_COOLDOWN

                    if self.world.healths[closest_enemy] <= 0:
                        logger.info(f"Soldier {closest_enemy} eliminated")
                    
                        # عند القضاء على عدو، تحقق إذا كانت الوحدة تحتاج إلى هدف جديد
                        unit_id = self.world.unit_ids[i]
                        self._check_unit_target(unit_id)
    
    def _check_unit_target(self, unit_id: int):
        """التحقق مما إذا كانت الوحدة تحتاج إلى تحديث هدفها بعد القضاء على عدو"""
        mask = self.world.unit_ids == unit_id
        if not np.any(mask):
            return
        
        # إذا كانت جميع الجنود في حالة ATTACKING ولكن لا يوجد أعداء، العودة إلى IDLE
        if np.all(self.world.states[mask] == UnitState.ATTACKING):
            # البحث عن أعداء جدد في نطاق أوسع
            unit_positions = self.world.positions[mask]
            centroid = np.mean(unit_positions, axis=0)
            
            nearby = self.world.quadtree.query_radius((centroid[0], centroid[1]), AI_SEARCH_RADIUS)
            
            has_enemies = False
            for x, y, idx in nearby:
                if (self.world.teams[idx] != self.world.teams[mask][0] and 
                    self.world.healths[idx] > 0):
                    has_enemies = True
                    break
            
            if not has_enemies:
                # لا توجد أعداء - العودة إلى حالة IDLE
                self.world.states[mask] = UnitState.IDLE
                self.world.units_need_reform.add(unit_id)
...
class AISystem:
    ...
    def update(self, delta_time: float):
        ...
        # تجميع الجنود الأحياء حسب الوحدة
        units = {}
        for i in range(self.world.soldier_count):
            if self.world.healths[i] <= 0:
                continue
            
            unit_id = int(self.world.unit_ids[i])
            if unit_id not in units:
                units[unit_id] = {
                    'indices': [],
                    'team': self.world.teams[i],
                    'positions': [],
                    'states': []
                }
            units[unit_id]['indices'].append(i)
            units[unit_id]['positions'].append(self.world.positions[i])
            units[unit_id]['states'].append(self.world.states[i])
        
        # تحديد علاقات الأعداء
        enemy_map = {
            Team.PLAYER: [Team.ENEMY],
            Team.ENEMY: [Team.PLAYER, Team.ALLY],
            Team.ALLY: [Team.ENEMY]
        }
        
        # معالجة كل وحدة ككيان واحد
        for unit_id, unit_data in units.items():
            if not unit_data['indices']:
                continue
            
            my_team = Team(unit_data['team'])
            enemy_teams = enemy_map.get(my_team, [])
            
            # حساب مركز الوحدة
            unit_positions = np.array(unit_data['positions'])
            centroid = np.mean(unit_positions, axis=0)
            ...
            # البحث عن أقرب عدو للوحدة (وليس للأفراد)
            for enemy_team in enemy_teams:
                # البحث في نطاق أوسع للوحدة ككل
                nearby = qt.query_radius((centroid[0], centroid[1]), AI_SEARCH_RADIUS * 1.5)
                ...
                for x, y, idx in nearby:
                    if (self.world.teams[idx] in enemy_teams and 
                        ...
                        # تحقق إذا كان أي جندي في الوحدة يمكنه مهاجمة هذا الهدف
                        for soldier_idx in unit_data['indices']:
                            soldier_pos = self.world.positions[soldier_idx]
                            attack_range = self.world.attack_ranges[soldier_idx]
                            dist_to_soldier_sq = (x - soldier_pos[0]) ** 2 + (y - soldier_pos[1]) ** 2
                            
                            if dist_to_soldier_sq <= attack_range ** 2:
                                target_in_attack_range = True
                                break
            
            # تحديث حالة الوحدة ككل
            if closest_target is not None:
                enemy_pos = self.world.positions[closest_target]
                
                if target_in_attack_range:
                    # الوحدة في مدى الهجوم - تهاجم كوحدات فردية
                    for soldier_idx in unit_data['indices']:
                        self.world.target_positions[soldier_idx] = enemy_pos
                        self.world.states[soldier_idx] = UnitState.ATTACKING
                        # إعادة تعيين وقت الخمول عند الاشتباك
                        self.world.state_timers[soldier_idx] = 0.0
                    
                    # إزالة الوحدة من قائمة إعادة التشكيل أثناء القتال
                    self.world.units_need_reform.discard(unit_id)
                
                else:
                    # الوحدة ليست في مدى الهجوم - تتحرك ككيان واحد نحو العدو
                    self._move_unit_toward_enemy(unit_id, unit_data['indices'], enemy_pos)
                    
                    # تغيير حالة جميع أفراد الوحدة إلى MOVING
                    for soldier_idx in unit_data['indices']:
                        self.world.states[soldier_idx] = UnitState.MOVING
                        self.world.state_timers[soldier_idx] = 0.0
            else:
                # لا يوجد أعداء - العودة إلى حالة الخمول
                all_idle = all(state == UnitState.IDLE for state in unit_data['states'])
                
                if all_idle:
                    # إضافة flag لإعادة التشكيل إذا كانت الوحدة خاملة
                    if unit_id not in self.world.units_need_reform:
                        self.world.units_need_reform.add(unit_id)
                else:
                    # تغيير حالة الأفراد المهاجمين إلى IDLE
                    for soldier_idx in unit_data['indices']:
                        if self.world.states[soldier_idx] == UnitState.ATTACKING:
                            self.world.states[soldier_idx] = UnitState.IDLE
                            self.world.state_timers[soldier_idx] = 0.0
    
    def _move_unit_toward_enemy(self, unit_id: int, soldier_indices: List[int], enemy_pos: np.ndarray):
        """تحريك الوحدة ككيان واحد نحو العدو مع الحفاظ على التشكيل"""
        if not soldier_indices:
            return
        
        # حساب مركز الوحدة الحالي
        unit_positions = self.world.positions[soldier_indices]
        centroid = np.mean(unit_positions, axis=0)
        
        # حساب الاتجاه نحو العدو
        direction = enemy_pos - centroid
        distance_to_enemy = np.linalg.norm(direction)
        
        if distance_to_enemy < 1.0:  # تجنب القسمة على صفر
            return
        
        direction_normalized = direction / distance_to_enemy
        
        # تحديد موقع الهدف للوحدة (على مسافة هجومية من العدو)
        attack_range = np.mean(self.world.attack_ranges[soldier_indices])
        stop_distance = attack_range * 0.8  # التوقف عند 80% من مدى الهجوم
        
        if distance_to_enemy <= stop_distance:
            # الوحدة قريبة بما يكفي - التوقف والهجوم
            return
        
        # حساب موقع الهدف للوحدة
        move_distance = min(distance_to_enemy - stop_distance, AI_SEARCH_RADIUS * 0.3)
        unit_target = centroid + direction_normalized * move_distance
        
        # التأكد من أن الهدف داخل حدود العالم
        unit_target[0] = np.clip(unit_target[0], 0, self.world.width)
        unit_target[1] = np.clip(unit_target[1], 0, self.world.height)
        
        # حساب الإزاحة للحفاظ على التشكيل
        current_offsets = unit_positions - centroid
        
        # تحديث أهداف جميع أفراد الوحدة مع الحفاظ على التشكيل النسبي
        new_targets = unit_target + current_offsets
        
        for i, soldier_idx in enumerate(soldier_indices):
            self.world.target_positions[soldier_idx] = new_targets[i]
...
class World:
    def __init__(self, width=800, height=600):
        ...
        self.deployment_zone = {
            Team.PLAYER: (50, 50, 300, 500),
            Team.ALLY: (width - 350, 50, 300, 500),
            Team.ENEMY: (width // 2 - 50, 50, 300, 300)
        }
